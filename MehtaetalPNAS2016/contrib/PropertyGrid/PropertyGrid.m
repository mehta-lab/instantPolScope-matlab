...
classdef PropertyGrid < UIControl
    ...
    methods
        ...
        function UpdateFieldsFromBoundItem(self, names)
        % Update specific properties by reading them from the bound item.
        % Added by Shalin Mehta (mshalin@mbl.edu) for occasions when the
        % bound Item is directly written to and GUI needs to be updated.
         for idn=1:numel(names) % For each field
             name=names{idn};
             field = self.Fields.FindByName(name);
             value= self.BoundItem.(name);
             if field.CanAccept(value)
                field.Value = value;
                field.PropertyData.Value=value;
                self.UpdateDependentProperties(field);
             else
                 self.BoundItem.(name)=field.PropertyData.Value;
                 warning(['Value of parameter:' name ' in the bound Item is illegal. It has been reset.']); 
             end
         end
        end
        
        function UpdateFields(self, names, values)
        % Update specific properties.
        % Added by Shalin Mehta (mshalin@mbl.edu) for occasions when the
        % properties need to be updated manually.
        if (~iscell(names)|| ~iscell(values) || numel(names)~=numel(values))
            error('Names and values need to be cell vectors of the same length.');
        end
         for idn=1:numel(names) % For each field
             name=names{idn};
             field = self.Fields.FindByName(name);
    ...
    methods (Access = private, Static)  % methods (Access = private, Static) for MatLab 2010a and up
