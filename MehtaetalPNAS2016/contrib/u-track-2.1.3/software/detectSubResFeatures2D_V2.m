...
%             .bgAmp     : Background intensity [Abg dAbg]. % returned by
%             Shalin.
...
for i = 1 : numClusters
    ...
    if keepCluster(i)
        ...
        clustersMMF(i).bgAmp =repmat(bgAmp,[size(maximaAmp,1) 1]);% bgAmp;
...
if ~isempty(indx)
    ...
    for iCluster = 1 : length(clustersMMF)
        ...
        pValue = 1 - fcdf(testStat,numDegFree,numDegFree+3*size(clustersMMF(iCluster).bgAmp(1,:),1));
    ...
    if ~isempty(indx)
        ...
        detectedFeatures.bgAmp = vertcat(clustersMMF.bgAmp); % Background information returned by Shalin.
    end
